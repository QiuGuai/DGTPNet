...
class GaussianAffinePatchHead(nn.Module):
    def __init__(self, input_dims, num_patch_points=16, hidden_dim=512, learnable_template=False):
        super().__init__()
        self.input_dims = input_dims
        self.num_patch_points = num_patch_points

        # predict offset(3) + log_scale(3) + quat(4)
        ...
        self.residual_head = nn.Sequential(
            nn.Linear(input_dims, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, num_patch_points * 3)
        )

        template = self.build_template(num_patch_points)   # (S, 3)
        if learnable_template:
            self.template = nn.Parameter(template)
        else:
            self.register_buffer('template', template)

    def build_template(self, num_patch_points):
        side = int(num_patch_points ** 0.5)
        assert side * side == num_patch_points, 'num_patch_points should be square, e.g. 16'
        ...
        template = torch.stack([grid_x, grid_y, z], dim=-1).reshape(-1, 3)
        template = template * 0.05   
        return template.float()

    def forward(self, feature, coarse_points):
        """
        feature: B M C
        coarse_points: B M 3
        return:
            patch_points: B M S 3
            aux: dict
        """
        B, M, _ = feature.shape
        S = self.num_patch_points

        params = self.param_head(feature)  # B M 10
        residual = self.residual_head(feature).reshape(B, M, S, 3)

        offset = 0.1 * params[..., 0:3]    # B M 3
        log_scale = params[..., 3:6]       # B M 3
        quat = params[..., 6:10]           # B M 4

        # Stability: Limiting the scale range
        log_scale = torch.clamp(log_scale, min=-4.0, max=1.0)
        scale = torch.exp(log_scale)       # B M 3

        rot = quat_to_rotmat(quat.reshape(-1, 4)).reshape(B, M, 3, 3)  # B M 3 3

        # final center = coarse anchor + offset
        mean = coarse_points + offset      # B M 3

        # template: S 3 -> B M S 3
        template = self.template.view(1, 1, S, 3).expand(B, M, S, 3)

        # anisotropic scaling
        scaled_template = template * scale.unsqueeze(-2)   # B M S 3

        # rotation
        rotated_template = torch.matmul(
            rot.unsqueeze(-3),                         # B M 1 3 3
            scaled_template.unsqueeze(-1)              # B M S 3 1
        ).squeeze(-1)                                  # B M S 3

        patch_points = mean.unsqueeze(-2) + rotated_template + residual  # B M S 3
...
class DGTPNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        assert self.decoder_type in ['gaussian'], f'unexpected decoder_type {self.decoder_type}'

        self.fold_step = 8
        self.base_model = PCTransformer(config)
        
        if self.decoder_type == 'gaussian':
            assert self.num_points is not None
            self.factor = self.num_points // self.num_query
            assert self.num_points % self.num_query == 0
            self.decode_head = GaussianAffinePatchHead(
                input_dims=self.trans_dim,
                num_patch_points=self.factor,
                hidden_dim=512,
                learnable_template=True
            )
        self.increase_dim = nn.Sequential(
    ...
    def forward(self, xyz):
        ...
        if self.decoder_type == 'fold':
            ...
            rebuild_feature = self.reduce_map(rebuild_feature)    # B M C
            rebuild_points = self.decode_head(
                rebuild_feature, coarse_point_cloud
            )                                                     # B M S 3
